...
schema, so an extra term never costs a repair call. Seed terms (from the preset or the admin) always come
first.
...
async def expand_question(
    ...
    *,
    ...
    """Keywords for `question`. `seed` defaults to the question's current keywords (the preset seed on
    create).
...
def apply_expansion(question: QuestionConfig, expansion: QuestionExpansion) -> QuestionConfig:
    """Same question with the expanded search terms.

    The version does not change: search terms do not change the meaning of the question.
    """
