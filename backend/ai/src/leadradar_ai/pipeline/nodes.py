...
def build_collect_request(services: list[ServiceBundle], now, max_items: int) -> CollectRequest:
    """One collection for all services: union of sources, the longest window, focus terms for news and jobs."""
...
class Nodes:
    ...
    async def resolve(self, state: AnalysisState) -> dict:
        ...
        except Exception as e:  # noqa: BLE001 — degrade: analyse with the given profile
    ...
    async def collect(self, state: AnalysisState) -> dict:
        ...
        except Exception as e:  # noqa: BLE001 — degrade: analyse with what we have
