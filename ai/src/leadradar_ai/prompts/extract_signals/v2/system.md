...
6. strength is the signal temperature and must follow the <temperature> guide of the question: evidence that fits
   cold → "weak", medium → "moderate", hot → "strong". When the evidence fits none of them, use the generic rule:
   strong = explicit and specific (named program, budget, target, date, open roles, appointed person);
