...
- Золотой набор: `src/leadradar_ai/evals/golden/mvp.jsonl` (метки) + `companies.yaml` (профили и фикстуры).
  Метки ставим **по содержимому фикстуры**. Сейчас 21 метка на 4 компаниях: 9 из Annex (DHL, Lufthansa) и 12 по
  фикстурам (Lufthansa, ловушки SAP — вендор и Orange — омоним). Живой прогон 2026-09-26: precision 0.90, recall 0.56,
  выдуманных цитат 0. Цель — ≥ 100 меток на ≥ 10 компаниях.
...
| Скоринг | Калибровка по ревью: τ_intent 3 → 5, степень Intent 0.6 → 0.8, Hot требует ≥ 2 вопросов с s_q ≥ 0.5; Fit nice-to-have = 40 + 60 × доля; недатированный сигнал × 0.5 | Веса почти не влияли на тир, один сигнал давал Hot; nice-to-have работал как must-have |
| Скоринг | Пример DHL из §1.7.5 при новых параметрах: Intent 63.4, Risk 38.1, Priority 54.4 | Числа в SPEC — для старой калибровки |
| Верификация | V2 в коде: на сторонних источниках имя компании (с учётом регистра) в заголовке или ≤ 250 символов от цитаты; V3: дата от LLM не позже даты статьи; V5: перепечатки одной истории схлопываются (`corroborated`) | Омоним Orange проходил проверку LLM; выдуманная свежая дата обходила свежесть и затухание; 10 перепечаток считались 10 доказательствами |
| NIS2/DORA | Производные сигналы сохраняются (детерминированный id, без документа) через `AnalysisStore.sync_derived`; отклонение пользователем сохраняется при пересчёте | Их не было видно в карточке и нельзя было отклонить |
| Источники | `registry` (Wikidata, Crunchbase) не добавлен в вопросы: это анкетные данные (сотрудники, отрасль), они идут в Fit и NIS2/DORA, а не в доказательства | Иначе промпт засоряется JSON-анкетами |
