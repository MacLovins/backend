...
| CO-A3 | Надстройка: HubSpot — `POST /leads/{company_id}/push/hubspot` (только ручная отправка, private app token, `FEATURE_HUBSPOT`); `GET /integrations/hubspot` для кнопки | После ядра | `pytest core/tests/test_hubspot.py` |
