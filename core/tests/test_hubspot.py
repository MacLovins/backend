...
def snapshot(**overrides) -> LeadSnapshot:
    data = {
        ...
        "intent": 80.0,
        "risk": 0.0,
        "why_now": [Reason("Runs agentic AI in customer service", "reuters", "https://news.example.com/r")],
        "evidence": [
            Evidence(
...
def test_note_escapes_html_and_links_the_evidence() -> None:
    ...
    assert "HOT (71/100)" in body and "WARM" not in body
    assert (
        '<li>Runs agentic AI in customer service <a href="https://news.example.com/r">reuters</a></li>'
        in body
    )
    assert "Open in LeadRadar" in body
    assert "WARM → HOT (71/100)" in note_body(snapshot(tier_before="warm"))
...
async def test_push_sends_active_evidence_and_remembers_the_record(client, hubspot) -> None:
    ...
    async with async_session_factory() as session:
        ...
        assert event.payload == {
            ...
            "hubspot_company_id": "901",
            "record_url": "https://app-eu1.hubspot.com/contacts/4242/record/0-2/901",
        }
...
async def test_unscored_lead_and_bad_token_are_reported(client, hubspot) -> None:
    ...
    res = client.post(f"/api/v1/leads/{company_id}/push/hubspot", headers=headers(org_id))
    assert res.status_code == 502 and "token" in res.json()["error"]["message"]

    hubspot.post("/crm/v3/objects/companies/search").respond(429, json={"message": "slow down"})
    res = client.post(f"/api/v1/leads/{company_id}/push/hubspot", headers=headers(org_id))
    assert res.status_code == 503
