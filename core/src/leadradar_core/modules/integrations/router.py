...
from leadradar_core.modules.integrations.hubspot import Evidence, HubSpotClient, HubSpotError, LeadSnapshot
...
async def _snapshot(
    ...
    lead_url = f"{settings.PUBLIC_ORIGIN.rstrip('/')}/companies/{company.id}?service={score.service_id}"
    snapshot = LeadSnapshot(
        ...
        why_now=[r["text"] for r in (score.why_now or []) if isinstance(r, dict) and r.get("text")][
            :MAX_REASONS
        ],
        evidence=[
            Evidence(
...
async def push_to_hubspot(
    ...
    try:
        async with HubSpotClient(settings.HUBSPOT_PRIVATE_APP_TOKEN) as hubspot:
