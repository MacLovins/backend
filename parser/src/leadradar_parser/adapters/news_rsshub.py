...
class RSSHubAdapter:
    """News feeds from a self-hosted RSSHub instance.

    Enabled only when RSSHUB_BASE_URL is set: the public rsshub.app answers automated clients with a
    Cloudflare challenge (HTTP 403), so it cannot be a default source.
    """
    ...
    async def fetch(
        ...
        queries = [f"{name} AI automation", f"{name} cybersecurity digital transformation"]
        failures: list[Exception] = []
        succeeded = emitted = 0
        seen: set[str] = set()
        for query in queries:
            feed_url = f"{base_url}/google/news/{quote(query)}"
            try:
            ...
            for item in items:
                ...
                yield make_document(
                    ...
                    meta={"feed": feed_url, "query": query, "headline_only": True},
                )
                emitted += 1
