...
| HubSpot (push компании, свойств скора и заметки с доказательствами) | `core.modules.integrations` — **сделано**: ручная кнопка `POST /leads/{id}/push/hubspot`, `GET /integrations/hubspot`; автопуш по `lead.tier_changed` не делаем | Outbox, CSV-экспорт, формат `why_now` | 3–4 ч |
